    ...
    private void InstantiateRandomCharactersOntoSpawnPositions( CharacterStats[] characterLibrary, Transform[] spawnPositions, List<CharacterStats> activeList )
    {
        ...
        */
        ...
        foreach( Transform sp in spawnPositions )
        {
            ...
            CharacterStats[] newCharacter = characterLibrary;
            int RandNum = Random.Range(0, newCharacter.Length);
            //Instantiate(newCharacter[i], sp.transform.position, Quaternion.identity);

            // Fix the new character GameObject name (eg remove the "(Clone)" Unity puts at the end)  Uncomment the next line...
            CharacterStats newCharacterInstance = Instantiate(newCharacter[RandNum], sp.transform.position, Quaternion.identity);
            newCharacterInstance.name = newCharacterInstance.name.Replace( "(Clone)", "" );
            //newCharacter[i].name = newCharacter[i].name.Replace( "(Clone)", "" );
           ...
           // activeList.Add( newCharacter[RandNum].GetComponent<CharacterStats>() );
            activeList.Add(newCharacterInstance);
        }
    }
